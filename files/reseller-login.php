...
if (empty($fatalMsg)) {
    // keep columns minimal to avoid conflicts; creation is idempotent
    $conn->query("
    ...
    ");
    $conn->query("
        CREATE TABLE IF NOT EXISTS remember_tokens_resellers (
...
/* remember-me (resellers) */
function create_remember_token_reseller(mysqli $conn, int $resellerId): string {
    $selector  = bin2hex(random_bytes(9));
    $validator = bin2hex(random_bytes(32));
    $hash      = hash('sha256', $validator);
    $expires   = date('Y-m-d H:i:s', time() + REMEMBER_TTL);

    $stmt = $conn->prepare("INSERT INTO remember_tokens_resellers (reseller_id, selector, hashed_validator, expires_at) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("isss", $resellerId, $selector, $hash, $expires);
    $stmt->execute();
    $stmt->close();

    return $selector . ':' . $validator;
}
function rotate_remember_token_reseller(mysqli $conn, int $resellerId, string $oldSelector): string {
    $stmt = $conn->prepare("DELETE FROM remember_tokens_resellers WHERE reseller_id=? AND selector=?");
    $stmt->bind_param("is", $resellerId, $oldSelector);
    $stmt->execute();
    $stmt->close();
    return create_remember_token_reseller($conn, $resellerId);
}
function consume_remember_cookie_reseller(mysqli $conn): ?array {
    if (empty($_COOKIE['remember_reseller'])) return null;
    $parts = explode(':', $_COOKIE['remember_reseller'], 2);
    if (count($parts) !== 2) { clear_cookie('remember_reseller'); return null; }

    [$selector, $validator] = $parts;

    $stmt = $conn->prepare("SELECT reseller_id, hashed_validator, expires_at FROM remember_tokens_resellers WHERE selector=? LIMIT 1");
    $stmt->bind_param("s", $selector);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();

    if (!$row) { clear_cookie('remember_reseller'); return null; }
    if (strtotime($row['expires_at']) < time()) {
        $del = $conn->prepare("DELETE FROM remember_tokens_resellers WHERE selector=?");
        $del->bind_param("s", $selector);
        $del->execute();
        $del->close();
        clear_cookie('remember_reseller');
        return null;
    }

    $calc = hash('sha256', $validator);
    if (!hash_equals($row['hashed_validator'], $calc)) {
        $rid = (int)$row['reseller_id'];
        $zap = $conn->prepare("DELETE FROM remember_tokens_resellers WHERE reseller_id=?");
        $zap->bind_param("i", $rid);
        $zap->execute();
        $zap->close();
        clear_cookie('remember_reseller');
        return null;
    ...
    $newToken = rotate_remember_token_reseller($conn, (int)$row['reseller_id'], $selector);
    set_secure_cookie('remember_reseller', $newToken, REMEMBER_TTL);
    return ['reseller_id' => (int)$row['reseller_id']];
}
...
if (empty($fatalMsg) && empty($_SESSION['reseller_id'])) {
    $payload = consume_remember_cookie_reseller($conn);
    if ($payload) {
        $_SESSION['reseller_id'] = $payload['reseller_id'];
...
if (empty($fatalMsg) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errsGlobal) && empty($fieldErrors['email']) && empty($fieldErrors['password'])) {
        if (too_many_attempts($conn, $email_in, $ip)) {
            ...
            $stmt = $conn->prepare("SELECT reseller_id, password FROM resellers WHERE email = ? LIMIT 1");
            if ($stmt) {
                ...
                if ($result && $result->num_rows === 1) {
                    ...
                    if (password_verify($password_in, $reseller['password'])) {
                        $_SESSION['reseller_id'] = (int)$reseller['reseller_id'];

                        clear_attempts($conn, $email_in, $ip);

                        if ($remember) {
                            $token = create_remember_token_reseller($conn, (int)$reseller['reseller_id']);
                            set_secure_cookie('remember_reseller', $token, REMEMBER_TTL);
                        } else {
                            clear_cookie('remember_reseller');
                        }
...
<title>Reseller Login | Mashobane & Co</title>
...
<main class="auth-shell">
  <section class="panel-left">
    ...
    <p>Access your dashboard to upload and manage assets.</p>
    <a class="ghost-btn" href="resellerSignup.php">
  ...
  <section class="panel-right">
    <div class="card" role="region" aria-labelledby="login-title">
      <h2 id="login-title">Reseller Sign In</h2>
      <div class="subtitle">Use your reseller account email to continue</div>
      ...
      <div class="links">
        <p>New reseller? <a href="reseller-signup.php">Register</a></p>
        <p>User instead? <a href="user-login.php">User login</a></p>
      </div>

      <div class="small-note">Need help? Contact support.</div>
    </div>
  </section>
</main>
